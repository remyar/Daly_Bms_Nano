...
bool DalyBms::requestData(COMMAND cmdID, unsigned int frameAmount) // new function to request global data
{
    ...
    // send the packet
    Serial1.write(this->my_txBuffer, XFER_BUFFER_LENGTH);
    // first wait for transmission end
    Serial1.flush();
    //-------------------------------------------

    //-----------Recive Part---------------------
    /*uint8_t rxByteNum = */ Serial1.readBytes(this->my_rxFrameBuffer, XFER_BUFFER_LENGTH * frameAmount);
...
bool DalyBms::sendCommand(COMMAND cmdID)
{
    ...
    {
        char t __attribute__((unused)) = Serial1.read(); // war auskommentiert, zum testen an
...
bool DalyBms::receiveBytes(void)
{
    ...
    uint8_t rxByteNum = Serial1.readBytes(this->my_rxBuffer, XFER_BUFFER_LENGTH);
